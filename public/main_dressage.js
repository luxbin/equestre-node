...
$(function() {
    ...
    socket.on('judges', function(data) {
        console.log("[on] judges:" + data.length /* + JSON.stringify(data) */ );
        judges = {};
        for (let judge of data) {
            judges[judge.position] = judge;
        }
        // update UI
        updateJudgeList();
    });
        ...
        for (i = limit - 1; i >= index; i--) {
            ...
            if (!ranking) {
                ...
                if (data[6] == '') {
                    ...
                    let startlistentry = startlistmap[num];
                    data[6] = tickToTimeD(startlistentry.start_time, 2);
                }
    ...
    function updateJudgeList() {
        ...
        for (let key in judges) {
            ...
            const col1 = $(`<td class="col-rank text-center bg-white text-color-black px-02 col-num-lg">${judge.index}</td>`);
            cols.push(col1);

            const col1_1 = $(`<td class="col-rank text-center bg-white text-color-black px-02 col-num-lg">${judge.position}</td>`);
            cols.push(col1_1);

            const col2 = $(`<td>${judge.name}</td>`);
            cols.push(col2);

            const nation = judge.nation.toLowerCase();
            const col3 = $(`<td style="background: #232323 url('/flags/${nation}.bmp') center no-repeat; background-size: contain;"></td>`);
            cols.push(col3);

            const col4 = $(`<td>${judge.code}</td>`);
            cols.push(col4);
    ...
    function updateHeaders(header) {
        ...
        tables.forEach(tableName => {
            ...
            if (tableName === 'startlist') {
                ...
                header[0] = header[1];
                header[1] = temp;
                header[6] = "<span data-key='TIME'>Time</span>"
            }
